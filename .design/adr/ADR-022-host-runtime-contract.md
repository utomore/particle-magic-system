...
- **同一 handle**：庫以原子的 read-modify-write 保證**不丟更新**（兩個 `pm_advance` 併發不會只推進一步），但**不保證順序**——推進與觀測的相對順序是宿主的責任。單執行緒宿主不付鎖的成本：每幀路徑無鎖，原子讀改寫的固定成本為個位數奈秒（實作後重測：單執行緒 +6.0 ns、8 執行緒 +8.8 ns，對照非原子寫入 2.62／4.60 ns）。
