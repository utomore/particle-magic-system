...
| [0008](docs/func-spec/0008-ortho2d-backend.md) | 2D 正交後端：`ViewPlane`/`orthographic`/`depthOrder` 入 `Magic.Project`、boundary 再匯出 `Magic.Projection`、真 2D 繪製路徑（painter 排序＋螢幕座標）、demo Tab/V 切換三視圖；兌現 ADR-0008 | 一般 | 無（0001/0002/0005 已完成）；**與 0007 平行**（檔案零交集，見其 §0.2） | 設計定案，待實作 |
