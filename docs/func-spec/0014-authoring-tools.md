...
| S3 | `ScanDir` op＋IO 節流＋`mergeSpellList`＋Loop 接線 | `test/RescanSpec.hs`（headless 腳本：新增→清單長、刪當前→clamp 相鄰、選擇依路徑保持、節流語意） |
...
（實作時回填：日期、`cabal test` 結果、CLI 對 assets 的實際輸出樣本；凍結清單：`magic-validate` 命令列介面與行格式、`ScanDir` op。）
