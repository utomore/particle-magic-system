...
> 前置依賴：**無**（spec 0001／0002／0005 皆已完成）。**與 spec 0007 平行**：0007（設計定案，待實作）鎖定 `src/core/Magic/{Rune,Circle,Compile}.hs`、`Magic/Particle/Analytic.hs`、`src/boundary/Magic/{Codec,Interface}.hs` 與新檔 `Magic/Particle/Field.hs`——本 spec 檔案清單與之**零交集**（§0.2 附盤點證明），兩 spec 可同時認領實作。
