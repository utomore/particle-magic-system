...
**新增**：`test/PersistEnvelopeSpec.hs`、`test/PersistWiringSpec.hs`、`test/Acceptance17Spec.hs`、`docs/adr/0015-sigil-persists-through-cast.md`。
...
| `docs/architecture.md`、`docs/spell-schema.md`、`docs/func-spec/0006-*.md`、`docs/adr/0014-*.md` | §0.3 的修訂註記（S3） |
...
4. **`docs/func-spec/0006-*.md` §4.4 共同欄位一併加了 0016 的修訂註記**（§0.3 只列了 §4.3）。該表的幾何欄位早在 0016 就被整體取代，但當時只在 0016 內部記錄；既然這輪要動同一份文件的相鄰段落，順手把它補上比留一個已知過期的表好。
