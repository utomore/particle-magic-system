...
對應主架構的 POC 實作階段，並承擔 §7 的全部效能目標。內部里程碑三個：**M1 兩層模型成立**（解析層 ＋ 力場層各自可用且組合語意釘死）、**M2 達到吞吐目標**（10k–100k）、**M3 多核與新欄**（平行取樣、速度欄的 opt-in 加欄範式）。三者皆已達成，`budgetCap` 也自 4096 抬到 16384。
...
|---|---------|-----------|------|------|
| 1 | buffer-and-step | SoA 緩衝與固定時步規劃器落地，走通端到端骨架 | - | func-0001 |
| 2 | analytic-sampling | 形狀取樣、軌跡、包絡排程組成解析層 `sample`（語意半場見 subarch-0001） | #1 | func-0002 |
| 3 | force-field-layer | 力場層落地：三種場、穩定槽位身分、加法位移疊加、零場結構性跳過 | #2 | func-0007 |

### 階段二：吞吐目標（M2，已交付）

| # | feature | 一句話說明 | 依賴 | spec |
|---|---------|-----------|------|------|
| 4 | performance-budget | count-then-fill、per-emitter 提升、時間窗剔除、`FieldState` SoA 化、結構化 `ParticleBudget` | #3 | func-0010 |

### 階段三：多核與新欄（M3，已交付）

| # | feature | 一句話說明 | 依賴 | spec |
|---|---------|-----------|------|------|
| 5 | parallel-sampling | 逐發射器分片的純 Strategies 平行取樣，決定論由切分方式結構性保證 | #4 | func-0022 |
| 6 | velocity-columns | 六欄 → 九欄的 opt-in 加欄範式：進迴圈前選建構子，無新欄者零代價 | #5 | func-0023 |

### 階段四：候選（未動工，逐條有記帳來源）

| # | feature | 一句話說明 | 依賴 | spec |
|---|---------|-----------|------|------|
| 7 | drop-intermediate-v3 | 去掉取樣器每粒子約 460 B 的中介 `V3` 配置——目前已知的下一個瓶頸（0022 §9 記帳） | #5 | - |
| 8 | budget-cap-raise | 依 ADR-0012 D7 的同一條規則重新量測並提高 `budgetCap`，同步 `pmMaxParticles`（ADR-0017 D5：前提已到位，屬另一輪） | #7 | - |
| 9 | velocity-aware-fields | 阻尼／磁力等速度相依的場——需 `fieldAccel` 看到速度，等於改熱路徑簽名並重寫每槽讀取（0021 §後記建議入主架構 §11） | #3 | - |
| 10 | caster-relative-fields | 施法者相對座標系的力場（0007 §9 記帳） | #3 | - |
| 11 | leased-buffer-api | 租借式緩衝／arena 的**新 API 合約**——純介面下的跨幀重用做不到，要做得換合約（0010 §9.4-10） | #4 | - |

小結：共 **11 個 features、4 個階段**，前 6 個已交付，吞吐目標與多核加速均已實測達成；階段四五項中 #7／#8 是同一條線（先去配置再抬上限），#9／#10 是力場層的語意擴充，#11 明列為「需要不同的 API 合約」而非優化。
