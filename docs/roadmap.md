...
> 版本：1.4（2026-08-15，**spec 0014（E＝作者工具）驗收後**更新 §1／§3.5／§5；1.3 為第一波 0010／0011／0013 三份同時驗收後的修訂，1.1 為 spec 0010–0014 設計定案後的修訂，1.0 為 spec 0001–0009 全數交付後的第一次全面盤點）
...
- **已交付**：func-spec 0001–0011、**0013** 與 **0014** 全數狀態「已完成」；ADR 0001–0011。第一波（0010 效能 ‖ 0011 宿主整合面 ‖ 0013 視覺）三份由三個 session 平行實作、同時驗收；第二波的 **0014（作者工具）**與 0012 平行，前者已驗收。
- **驗證**：`cabal build all` 綠（含 exe、**新的 `magic-validate` exe**、bench、foreign-library）；`cabal test` **952 examples, 0 failures**（0014 交付後；三份合併時為 903，0009 交付時為 676）。0007／0008／0009 曾以同一模式三方平行；0010／0011／0013 再次驗證零檔案交集切分成立，0014 對 0012 的零交集亦事後成立（見 0014 §9.5）。
...
## 5. 建議的下一步（1.4 修訂：0014 驗收，第二波只剩 0012）
...
- **第二波**：~~0014 = E（作者工具）~~ ✅（2026-08-15 驗收）‖ **0012 = C（多陣合成與場景層＋上限正式提升）——唯一未交付的一份**。0012 的兩個前置（0010 的 `ParticleBudget`＋§9.2 選值建議 32 768–65 536、0011 的查詢鏡射律契約鬆綁）都已在手，可立即動工／續作。

理由一句話：A 把速度做出來（已完成，取樣快 3.4×、100k 粒 6.5 ms）、B 把管道鋪出去、D 把畫面看清楚——三件事互不碰檔案；E 把作者回饋圈補完（已完成）；只剩 C 用 A 的量測與 B 的鬆綁把 4096 真正抬走。

**0012 之後**：本輪設計的五個候選全數交付，路線圖需要一次新的全面盤點（下一批候選來源：§3 剩餘各列、§6 的分水嶺清單，以及 §3.5 新增的兩項——CI 與發布流程，兩者的門檻都已因 0014 的 CLI 而降低）。
