...
> 版本：1.5（2026-08-15，spec 0016 符文陣設計定案：§3.4 新增陣形列、§4.7 加候選 F2、§4.8 依賴鏈延長、§5 第三波補述；1.4 為對 architecture.md 的逐節達成率盤點＋spec 0015 設計定案：新增 §2.1、§4.7，改寫 §5 為三波、更新 §3.4／§6；1.3 為第一波 0010／0011／0013 驗收後的修訂，1.1 為 spec 0010–0014 設計定案後的修訂，1.0 為 spec 0001–0009 全數交付後的第一次全面盤點）
...
- **進行中／已設計**：0012（多陣合成）與 0014（作者工具）已由兩個 session 認領實作；**0015（視覺表現力・核心半場）**與 **0016（符文陣）設計定案**，動工門檻分別為 0012 與 0015 驗收。共 16 份 spec 存在，其中 12 份已完成。
...
- **第二波（門檻已全部解除，兩份皆已認領實作）**：**0012 = C（多陣合成與場景層＋上限正式提升）** ‖ **0014 = E（作者工具）**。0012 的兩個前置（0010 的 `ParticleBudget`＋§9.2 選值建議 32 768–65 536、0011 的查詢鏡射律契約鬆綁）都已在手；0014 的前置（0010）與額外門檻（0013 驗收）亦已滿足。
- **第三波**：**0015 = F（視覺表現力・核心半場）**，設計定案、**動工門檻＝0012 驗收**（五檔交集），**與 0014 平行**。
- **第四波**：**0016 = F2（符文陣）**，設計定案、**動工門檻＝0015 驗收**（`Compile.hs` 交集），**與 0014 平行**。候選 G／H 未設計（§4.7）。

理由一句話：A 把速度做出來（已完成，取樣快 3.4×、100k 粒 6.5 ms）、B 把管道鋪出去、D 把畫面看清楚——三件事互不碰檔案；然後 C 用 A 的量測與 B 的鬆綁把 4096 真正抬走，E 把作者回饋圈補完；最後 F 讓畫面上的東西**值得看**——0013 解決的是「看得清楚」，0015 解決的是「看的是什麼」，0016 則讓發動前的那個陣變成**這個魔法自己的**陣。
...
3. **視覺表現力**（候選 D→F）——**觀察面已於 0013 交付**（3D alpha 深度排序、軌道相機、2D 平移縮放、俯視深度色調），現在看得清楚了；**表現面由 spec 0015 認領**（設計定案，門檻＝0012 驗收）：`BillboardShape` 由 1 個建構子擴為 4 個、外圈 `StyleRune` 讓魔法陣自己指定粒子形態、`observeSpell` 改多批次讓 `rbShape` 差異化真的有意義。命題是「特效即魔法」，而在 0015 之前，粒子形態這個語意維度的值域大小是 1。
