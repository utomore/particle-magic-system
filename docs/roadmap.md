...
> 版本：1.2（2026-08-14，spec 0011 驗收後更新 §1／§2／§3.3／§5；1.1 為 spec 0010–0014 設計定案後的修訂）
...
- **已交付**：func-spec 0001–0009 與 **0011** 狀態「已完成」；ADR 0001–0011。第一波剩 0010（效能）與 0013（視覺）。
- **驗證**：`cabal build all` 綠（含 exe、bench、foreign-library）；`cabal test` **706 examples, 0 failures**。0007／0008／0009 三份 spec 由三個 session 平行實作、依零檔案交集的切分各自合併進 main，合併後零回歸；0011 依同一模式與 0010／0013 平行。
...
- **第一波（三 session 可同時認領）**：**0010 = A（效能）** ‖ ~~0011 = B（宿主整合面）~~ ✅ **已驗收（2026-08-14）** ‖ **0013 = D（視覺，`app/*` 半場）**——三方檔案零交集，各 spec §0.2 附逐檔證明。
- **第二波（0010＋0011 驗收後）**：**0012 = C（多陣合成與場景層＋上限正式提升）** ‖ **0014 = E（作者工具；動工門檻另含 0013 驗收）**。0011 已交付，**0012 的動工門檻現在只剩 0010**。
