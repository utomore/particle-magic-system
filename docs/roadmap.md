...
> 版本：1.4（2026-08-15，對 architecture.md 的逐節達成率盤點＋spec 0015 設計定案：新增 §2.1、§4.7，改寫 §5 為三波、更新 §3.4／§6；1.3 為第一波 0010／0011／0013 驗收後的修訂，1.1 為 spec 0010–0014 設計定案後的修訂，1.0 為 spec 0001–0009 全數交付後的第一次全面盤點）
...
- **進行中／已設計**：0012（多陣合成）與 0014（作者工具）已由兩個 session 認領實作；**0015（視覺表現力・核心半場）設計定案**，動工門檻＝0012 驗收。共 15 份 spec 存在，其中 12 份已完成。
...
- **第三波**：**0015 = F（視覺表現力・核心半場）**，設計定案、**動工門檻＝0012 驗收**（五檔交集），**與 0014 平行**。候選 G／H 未設計（§4.7）。

理由一句話：A 把速度做出來（已完成，取樣快 3.4×、100k 粒 6.5 ms）、B 把管道鋪出去、D 把畫面看清楚——三件事互不碰檔案；然後 C 用 A 的量測與 B 的鬆綁把 4096 真正抬走，E 把作者回饋圈補完；最後 F 讓畫面上的東西**值得看**——0013 解決的是「看得清楚」，0015 解決的是「看的是什麼」。
