...
> 版本：1.4（2026-08-15，spec **0012** 驗收後更新 §1／§2／§3／§5／§6；1.3 為第一波 0010／0011／0013 同時驗收後的修訂，1.1 為 spec 0010–0014 設計定案後的修訂，1.0 為 spec 0001–0009 全數交付後的第一次全面盤點）
...
- **已交付**：func-spec 0001–0013 全數狀態「已完成」（只剩 **0014** 未動工）；ADR 0001–0012。第一波（0010 效能 ‖ 0011 宿主整合面 ‖ 0013 視覺）三份平行、同時驗收；第二波的 **0012（多陣合成與場景層）** 隨後交付。
- **驗證**：`cabal build all` 綠（含 exe、bench、foreign-library）；`cabal test` **969 examples, 0 failures**（0012 交付後；三份合併時為 903，0009 交付時為 676）。
...
| 工程紀律（測試／文件） | **95%** | Todo↔測試 1:1、逐位元相容律、golden 鎖定、契約守護測試（cabal／header／`.def`／C# 綁定四份文本）、12 ADR／13 spec 已完成／**969 examples** | 無 CI；只有 win64 實測過 |
...
## 5. 建議的下一步（1.4 修訂：0012 驗收，只剩 0014）
...
- **第二波**：~~**0012 = C（多陣合成與場景層＋上限正式提升）**~~ ✅（2026-08-15 驗收）‖ **0014 = E（作者工具）——唯一未動工的一份**，前置（0010）與額外門檻（0013 驗收）皆已滿足，可立即認領。0012 的檔案盤點事後成立（與 0014 零交集；0012 §9.5-5 另更新了五個測試檔中寫死的 4096，同樣不與 0014 相交）。

理由一句話：A 把速度做出來（取樣快 3.4×、100k 粒 6.5 ms）、B 把管道鋪出去、D 把畫面看清楚——三件事互不碰檔案；C 用 A 的量測與 B 的鬆綁把 4096 抬到 16384 並補上最後一列語意；剩下 E 把作者回饋圈補完。

**0012 之後，下一輪該蓋什麼？** 本文不做新決策，但盤點指向的是**視覺表現面的核心那半**（`BillboardShape` 新建構子、`rbShape` 差異化渲染，0013 §8-1）——維度 C（產品級特效系統）是三維中最低的一項，而「特效即魔法」這個命題正押在它身上。次選是 §3.3 的 `pm_scene_*`（等場景層在 Haskell 面被真實宿主用過一輪）。
