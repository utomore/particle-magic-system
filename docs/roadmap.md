...
> 版本：1.5（2026-08-15，第二波 spec **0012**（C＝多陣合成與場景層）與 **0014**（E＝作者工具）兩份平行驗收後合併更新 §1／§2／§3／§5／§6；1.3 為第一波 0010／0011／0013 同時驗收後的修訂，1.1 為 spec 0010–0014 設計定案後的修訂，1.0 為 spec 0001–0009 全數交付後的第一次全面盤點）
...
- **驗證**：`cabal build all` 綠（含 demo exe、**新的 `magic-validate` exe**、bench、foreign-library）；`cabal test` **1018 examples, 0 failures**（0012＋0014 合併後於整合分支實測；0012 單獨交付時 969、0014 單獨交付時 952，第一波三份合併時 903，0009 交付時 676）。0007／0008／0009 曾以同一模式三方平行；0010／0011／0013 再次驗證零檔案交集切分成立，0012 對 0014 的零交集亦事後成立（見 0012 §9.5、0014 §9.5）——本次整合只有 `docs/roadmap.md` 與 `.cabal` 的索引列需要人工併行。
...
## 5. 建議的下一步（1.5 修訂：第二波 0012／0014 全數驗收，0001–0014 全部交付）
...
理由一句話：A 把速度做出來（取樣快 3.4×、100k 粒 6.5 ms）、B 把管道鋪出去、D 把畫面看清楚——三件事互不碰檔案；C 用 A 的量測與 B 的鬆綁把 4096 抬到 16384 並補上最後一列語意；E 把作者回饋圈補完。

**下一輪該蓋什麼？** 本輪設計的五個候選全數交付，路線圖需要一次新的全面盤點；本文不做新決策，但盤點指向的是**視覺表現面的核心那半**（`BillboardShape` 新建構子、`rbShape` 差異化渲染，0013 §8-1）——維度 C（產品級特效系統）是三維中最低的一項，而「特效即魔法」這個命題正押在它身上。次選是 §3.3 的 `pm_scene_*`（等場景層在 Haskell 面被真實宿主用過一輪）。其餘候選來源：§3 剩餘各列、§6 的分水嶺清單，以及 §3.5 的 CI 與發布流程（兩者門檻都已因 0014 的 CLI 而降低）。
