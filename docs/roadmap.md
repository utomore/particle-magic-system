...
> 版本：1.2（2026-08-14，spec 0013 驗收後修訂 §1／§3.1／§3.4／§5／§6；1.1 為 spec 0010–0014 設計定案後的修訂，1.0 為 spec 0001–0009 全數交付後的第一次全面盤點）
...
- **已交付**：func-spec 0001–0009 全數狀態「已完成」，**0013 已完成**（第一波三份平行 spec 的視覺半場）；ADR 0001–0011。
- **驗證**：`cabal build all` 綠（含 exe、bench、foreign-library）；`cabal test` **756 examples, 0 failures**（0013 交付後；0009 交付時為 676）。0007／0008／0009 三份 spec 由三個 session 平行實作、依零檔案交集的切分各自合併進 main，合併後零回歸；0013 沿用同一結構（只碰 `app/*`）。
...
| ~~3D 路徑的深度排序~~ **已交付 0013**（`App.Render.Order.viewOrder`；同樣以 `sortOn` 起步，效能化記在 0013 §8-6） | 0005 §9、0008 §9-1 |
...
- **第一波（三 session 可同時認領）**：**0010 = A（效能）** ‖ **0011 = B（宿主整合面）** ‖ ~~**0013 = D（視覺，`app/*` 半場）**~~ **已於 2026-08-14 驗收完成**——三方檔案零交集，各 spec §0.2 附逐檔證明；0013 實際只動 `app/*` 與 `test/*`，連 `app/Main.hs` 都零修改（見其 §9.4-5）。
- **第二波（0010＋0011 驗收後）**：**0012 = C（多陣合成與場景層＋上限正式提升）** ‖ **0014 = E（作者工具；動工門檻另含 0013 驗收——**此門檻已解除**）**。
